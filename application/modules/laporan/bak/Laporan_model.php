...
class laporan_model extends CI_Model {
	...
	public function get_all_service_keluar($dari = '', $sampai = '')
	{
		if ($dari != '') {
			$query = "
			SELECT 
			DATE(tgl_service) AS tgl, 
			SUM(k) as keluar, 
			SUM(total_bayar) as pendapatan from (
			SELECT tgl_service, total_bayar, if(status = 'TELAH DITERIMA',1,0) as k from service WHERE DATE(tgl_service) BETWEEN '$dari' AND '$sampai'
			) as t
			group by DATE(t.tgl_service)
			";	
		}else{
			$query = "
			SELECT 
			...
			SUM(k) as keluar, 
			SUM(total_bayar) as pendapatan from (
			SELECT tgl_service, total_bayar, if(status = 'TELAH DITERIMA',1,0) as k from service
			) as t
			group by DATE(t.tgl_service)
			";	
		}
	...
	public function get_all_service_masuk($dari = '', $sampai = '')
	{
		if ($dari != '') {
			$query = "
			SELECT 
			SUM(m) as masuk from (
			SELECT tgl_service, total_bayar, if(status = 'BELUM DITERIMA',1,0) as m from service WHERE DATE(tgl_service) BETWEEN '$dari' AND '$sampai'
			) as t
			group by DATE(t.tgl_service)
			";	
		}else{
			...
			SELECT 
			SUM(m) as masuk from (
			SELECT tgl_service, total_bayar, if(status = 'BELUM DITERIMA',1,0) as m from service
			) as t
			group by DATE(t.tgl_service)
			";	
		}
	...
	public function get_total_service($dari = '', $sampai = '')
	{
		$this->db->select_sum('total_bayar', 'total');
		if ($dari != '') {
			$this->db->where('DATE(service.tgl_service) >=', $dari);
			$this->db->where('DATE(service.tgl_service) <=', $sampai);
		}
